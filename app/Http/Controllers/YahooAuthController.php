...
///////////////////////////////////////////////////////////////////////////////
    protected function _make_signed_request( $token, $token_secret, $signature_method, $url, $params = array())
    {
        ...
        // Generate secret
        $secret = urlencode(env("CONSUMER_SECRET")) . '&' . urlencode($token_secret);
...
///////////////////////////////////////////////////////////////////////////////
    protected function oauth_response_to_array($response)
    {
        $data = array();
...
///////////////////////////////////////////////////////////////////////////////
    protected function get_request_token()
    {
        ...
        $response_data = $this->_make_signed_request($token, $token_secret, $signature_method, $url, $params);
...
///////////////////////////////////////////////////////////////////////////////
    protected function get_access_token($request_token, $request_token_secret, $verifier)
    {
        ...
        $response_data = $this->_make_signed_request($request_token, $request_token_secret, $signature_method, $url, $params);

        if ($response_data && $response_data['return_code'] == 200) {

            $contents = $response_data['contents'];
            $data = $this->oauth_response_to_array($contents);
...
///////////////////////////////////////////////////////////////////////////////
    protected function make_request($access_token, $access_token_secret, $url)
    {

        $signature_method = 'hmac-sha1';

        // Make the signed request to fantasy API
        $response_data = $this->_make_signed_request($access_token, $access_token_secret, $signature_method, $url);
    ...
    public function getAccessToken()
    {
        ...
        $request_token_data = $this->get_request_token();

        return $this->accessToken($request_token_data);
    }

    protected function accessToken($request_token_data)
    {
...
// 3. Get Access Token
        $access_token_data =
            $this->get_access_token($request_token, $request_token_secret, $verifier);
...
// 4. Make request using Access Token
        ...
        if (isset($argv[1])) {
            ...
            $request_uri = 'fantasy/v2/game/nhl';
        }
        ...
        $request_data = $this->make_request( $access_token, $access_token_secret, $request_url);
